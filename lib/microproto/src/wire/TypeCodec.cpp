...
bool TypeCodec::encodeProperty(WriteBuffer& buf, const PropertyBase* prop) {
    ...
    if (typeId == TYPE_STREAM) {
        // STREAM: delegate to the property's encodeFullBuffer()
        // For broadcasts, the controller calls encodePendingEntries() directly
        return prop->encodeFullBuffer(buf);
    }
...
bool TypeCodec::decodeProperty(ReadBuffer& buf, PropertyBase* prop) {
    ...
    if (typeId == TYPE_RESOURCE || typeId == TYPE_STREAM) {
        // Resources and streams are read-only — clients cannot send updates via PROPERTY_UPDATE.
