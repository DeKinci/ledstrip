...
namespace Widget {
    ...
    enum class Number : uint8_t {
        ...
        SLIDER = 1,
        SPINBOX = 2,
        HUE_SLIDER = 3     // Rainbow gradient hue picker
    };
    ...
    enum class Stream : uint8_t {
        AUTO = 0,
        LOG = 1,            // Log viewer (filterable by level/tag)
        TIMESERIES = 2      // Time-series graph (gauge/counter)
    };
}
...
struct UIHints {
    ...
    UIHints& setWidget(Widget::Stream w) { widget = static_cast<uint8_t>(w); return *this; }
...
public:
    ...
    // Stream properties (override in StreamProperty)
    virtual bool isStream() const { return false; }
    virtual bool encodePendingEntries(WriteBuffer& buf) const { return false; }
    virtual bool encodeFullBuffer(WriteBuffer& buf) const { return false; }
