...
#else
#include <cstdio>
#define LOG_INFO(tag, ...) do { printf("[%s] ", tag); printf(__VA_ARGS__); printf("\n"); } while(0)
#define LOG_WARN(tag, ...) do { printf("[%s] WARN: ", tag); printf(__VA_ARGS__); printf("\n"); } while(0)
#define LOG_ERROR(tag, ...) do { printf("[%s] ERROR: ", tag); printf(__VA_ARGS__); printf("\n"); } while(0)
...
void MicroProtoController::flushBroadcasts() {
    ...
    while (batchStart < dirtyCount) {
        ...
        while (batchEnd < dirtyCount) {
            ...
            if (!wb.writePropId(dirtyProps[batchEnd]->id) ||
                !TypeCodec::encodeProperty(wb, dirtyProps[batchEnd])) {
    ...
    if (anyBleClient) {
        ...
        for (size_t i = 0; i < dirtyCount; i++) {
            ...
            size_t posBefore = wb.position();
            if (!wb.writePropId(dirtyProps[i]->id) ||
                !TypeCodec::encodeProperty(wb, dirtyProps[i])) {
                wb.setPosition(posBefore);
                break;
            }
            sent++;
