...
// seg:get(col, row) → color — matrix read (1-based)
static int segment_get(lua_State *L) {
    LuaSegment* seg = checkSegment(L);
    ...
    if (w == 0 || h == 0 || col < 0 || col >= w || row < 0 || row >= h) {
        lua_pushinteger(L, 0);
        return 1;
    }
    ...
    if (idx >= 0 && idx < seg->view->data->ledCount) {
        const CRGB& c = seg->view->leds[idx];
        lua_pushinteger(L, packColor(c.r, c.g, c.b));
    } else {
        lua_pushinteger(L, 0);
    }
...
static void registerSegmentMetatable(lua_State *L) {
    ...
    lua_pushcfunction(L, segment_get);
...
// Execution timeout hook — abort draw() if it runs too long
static uint32_t luaExecStart = 0;
static constexpr uint32_t LUA_EXEC_TIMEOUT_MS = 500;

static void luaTimeoutHook(lua_State *L, lua_Debug *) {
    if (millis() - luaExecStart > LUA_EXEC_TIMEOUT_MS) {
        luaL_error(L, "execution timeout (%dms)", LUA_EXEC_TIMEOUT_MS);
    }
}
...
CallResult<void *> LuaAnimation::apply(CRGB *leds, size_t size,
    ...
    luaExecStart = millis();
    lua_sethook(co, luaTimeoutHook, LUA_MASKCOUNT, 10000);  // check every ~10K instructions
    int status = lua_resume(co, luaState, nargs);
    lua_sethook(co, nullptr, 0, 0);  // clear hook
