...
class MicroProtoClient {
    ...
    constructor(url, options = {}) {
        ...
        // Schema caching
        this._schemaVersion = 0;  // Current server schema version (from HELLO response)
        this._storageKey = options.storageKey || ('mp_' + this._hashUrl(url));
    }
    ...
    _sendHello() {
        ...
        const cachedVersion = this._loadCachedSchemaVersion();

        const buf = new ArrayBuffer(2 + maxPacketSizeBytes.length + deviceIdBytes.length + 2);
        const bytes = new Uint8Array(buf);

        let offset = 0;
        bytes[offset++] = MicroProtoClient.OPCODES.HELLO;  // opcode=0, flags=0 (request)
        bytes[offset++] = MicroProtoClient.PROTOCOL_VERSION;
        bytes.set(maxPacketSizeBytes, offset);
        offset += maxPacketSizeBytes.length;
        bytes.set(deviceIdBytes, offset);
        offset += deviceIdBytes.length;
        // Schema version (u16 little-endian)
        bytes[offset++] = cachedVersion & 0xFF;
        bytes[offset++] = (cachedVersion >> 8) & 0xFF;

        this.ws.send(buf);
    }

    _sendPropertyUpdate(propertyId, value, typeId) {
        // Get property schema for complex types
        const prop = this.properties.get(propertyId);

        // Complex types: encode into a dynamic buffer using schema
        if (typeId === MicroProtoClient.TYPES.LIST ||
            typeId === MicroProtoClient.TYPES.OBJECT ||
            typeId === MicroProtoClient.TYPES.ARRAY) {
            const encoded = this._encodeTypedValue(value, prop);
            if (!encoded) {
                this._log('Failed to encode property', propertyId);
                return;
            }
            const propIdBytes = this._encodePropId(propertyId);
            const buf = new Uint8Array(1 + propIdBytes.length + encoded.length);
            buf[0] = MicroProtoClient.OPCODES.PROPERTY_UPDATE;
            buf.set(propIdBytes, 1);
            buf.set(encoded, 1 + propIdBytes.length);
            this._log('Sending complex property update:', propertyId, encoded.length, 'bytes');
    ...
    // Encode a typed value using property schema info
    _encodeTypedValue(value, prop) {
        if (!prop) return null;
        const parts = [];
        this._encodeTypedValueInto(parts, value, prop);
    ...
    _encodeTypedValueInto(parts, value, typeDef) {
        const typeId = typeDef.typeId;

        if (typeId === MicroProtoClient.TYPES.LIST) {
            const arr = Array.isArray(value) ? value : [];
            parts.push(this._encodeVarint(arr.length));
            for (const elem of arr) {
                this._encodeTypedValueInto(parts, elem, typeDef.elementTypeDef);
            }
            return;
        }
        ...
        if (typeId === MicroProtoClient.TYPES.ARRAY) {
            ...
            for (let i = 0; i < count; i++) {
                this._encodeTypedValueInto(parts, arr[i] || 0, typeDef.elementTypeDef);
            }
            return;
        }
        ...
        if (typeId === MicroProtoClient.TYPES.OBJECT) {
            ...
            for (const field of fields) {
                ...
                this._encodeTypedValueInto(parts, fieldVal, field.typeDef);
            }
            return;
        }
        ...
        const view = new DataView(buf);
        this._encodeValue(view, 0, value || 0, typeId);
    ...
    _handleHelloResponse(data, flags) {
        ...
        // Schema version (u16 little-endian)
        const serverSchemaVersion = data[offset] | (data[offset + 1] << 8);
        offset += 2;
        ...
        // Schema caching: if server version matches our cached version,
        // server will skip sending schema — keep existing property definitions.
        const cachedVersion = this._loadCachedSchemaVersion();
        const schemaMatch = cachedVersion !== 0 && cachedVersion === serverSchemaVersion;

        this._log('HELLO response: version=', version, 'session=', sessionId,
                  'schemaVersion=', serverSchemaVersion, schemaMatch ? '(cached)' : '(will resync)');
        this.connected = true;

        if (schemaMatch) {
            // Restore cached schema — server won't send SCHEMA_UPSERT messages
            if (!this._loadCachedSchema()) {
                // Cache corrupted — resync by sending fresh HELLO with version=0
                this._log('Cache restore failed, requesting resync');
                this._schemaVersion = 0;
                this._saveCachedSchemaVersion(0);
                this._sendHello();
                return;
            }
        } else {
            // Clear state — server will send fresh schema
        ...
        this._schemaVersion = serverSchemaVersion;
        this._saveCachedSchemaVersion(serverSchemaVersion);

        this._emit('connect', { version, maxPacket, sessionId, timestamp, schemaVersion: serverSchemaVersion, schemaMatch });
    }
    ...
    _handleSchemaUpsert(data, flags) {
        ...
        for (let i = 0; i < count && offset < data.length; i++) {
            ...
            const [item, newOffset] = result;
            offset = newOffset;

            if (item.type === 'function') {
                // Functions are already registered in _decodeSchemaItem
                this._log('  Function:', item.name, 'id=', item.id, 'params=', item.params.length);
            } else {
                this.properties.set(item.id, item);
                this.propertyByName.set(item.name, item.id);
                this._log('  Property:', item.name, 'id=', item.id, 'type=', item.typeId);
            }

            this._emit('schema', item);
        }

        // Cache schema for future connections
        this._saveCachedSchema();
    }
    ...
    _decodeSchemaItem(data, offset) {
        ...
        // Level flags (shared by properties and functions)
        ...
        if (propType === 2) {
            ...
            for (let i = 0; i < paramCount; i++) {
                // Parameter name (ident: u8 len + bytes)
                const pNameLen = data[offset++];
                const pName = new TextDecoder().decode(data.slice(offset, offset + pNameLen));
                offset += pNameLen;

                // Parameter DATA_TYPE_DEFINITION
                const pResult = this._decodeDataTypeDefinition(data, offset);
                if (!pResult) return null;
                const [pTypeDef, pOffset] = pResult;
                offset = pOffset;

                params.push({ name: pName, typeId: pTypeDef.typeId, typeDef: pTypeDef });
            }

            // Return type DATA_TYPE_DEFINITION
            const returnTypeId = data[offset++];
            let returnTypeDef = null;
            if (returnTypeId !== 0x00) {  // 0x00 = void
                // Read validation flags for basic return types
                const valFlags = data[offset++];
                returnTypeDef = { typeId: returnTypeId };
            }

            const func = {
                id, name, description, type: 'function',
                params, returnTypeId, returnTypeDef,
                bleExposed, namespaceId
            };

            // Register in function maps
            this.functions.set(id, func);
            this.functionByName.set(name, id);
    ...
    _getBasicTypeSize(typeId) {
        switch (typeId) {
            ...
            case MicroProtoClient.TYPES.UINT8:
                return 1;
            case MicroProtoClient.TYPES.INT16:
            case MicroProtoClient.TYPES.UINT16:
                return 2;
    ...
    _decodeDataTypeDefinition(data, offset) {
        ...
        if (typeId >= 0x01 && typeId <= 0x07) {
            // Basic types: BOOL, INT8, UINT8, INT32, FLOAT32, INT16, UINT16
    ...
    // =========== Schema Caching (localStorage) ===========

    _hashUrl(url) {
        // Simple FNV-1a hash of URL → 8 hex chars for localStorage key scoping
        let hash = 2166136261;
    ...
    _loadCachedSchemaVersion() {
        try {
            return parseInt(localStorage.getItem(this._storageKey + '_v') || '0', 10) || 0;
        } catch { return 0; }
    }

    _saveCachedSchemaVersion(version) {
        try { localStorage.setItem(this._storageKey + '_v', String(version)); }
        catch {}
    }
    ...
    _saveCachedSchema() {
        try {
            ...
            localStorage.setItem(this._storageKey, JSON.stringify(schema));
        } catch {}
    }
    ...
    _loadCachedSchema() {
        try {
            ...
            this.properties.clear();
            this.propertyByName.clear();
            for (const [id, prop] of schema) {
                this.properties.set(id, prop);
                this.propertyByName.set(prop.name, id);
            }
            this._log('Restored', this.properties.size, 'properties from cache');
            // Emit schema events so widget registry creates widgets
            for (const [id, prop] of this.properties) {
